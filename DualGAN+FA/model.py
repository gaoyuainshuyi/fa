...
class DualNet(object):
    ...
    def load_random_samples(self):
        #np.random.choice(
        sample_files =np.random.choice(glob('./datasets/{}/valA/*.jpg'.format(self.dataset_name)),self.batch_size)
        sample_A_imgs = [load_data(f, image_size =self.image_size, flip = False) for f in sample_files]
        
        sample_files = np.random.choice(glob('./datasets/{}/valB/*.jpg'.format(self.dataset_name)),self.batch_size)
    ...
    def train(self, args):
        ...
        for epoch_idx in xrange(args.epoch):
            data_A = glob('./datasets/{}/trainA/*.jpg'.format(self.dataset_name))
            data_B = glob('./datasets/{}/trainB/*.jpg'.format(self.dataset_name))
    ...
    def discriminator(self, image,  y=None, prefix='A_d_', reuse=False):
        ...
        with tf.variable_scope(tf.get_variable_scope()) as scope:
            ...
            h0 = conv2d(image, self.df_dim, name=prefix+'h0_conv')
            h0 = lrelu(h0)
            # h0 = attention(h0, name=prefix+'attention_dis_0')
            

            # h0 is (128 x 128 x self.df_dim)
            h1 = conv2d(h0, self.df_dim*2, name=prefix+'h1_conv')
            h1 = batch_norm(h1, name = prefix+'bn1')
            h1 = lrelu(h1)
            # h1 = attention(h1, name=prefix+'attention_dis_1')
            
            # h1 is (64 x 64 x self.df_dim*2)
            h2 = conv2d(h1, self.df_dim*4, name=prefix+'h2_conv')
            h2 = batch_norm(h2, name = prefix+ 'bn2')
            h2 = lrelu(h2)
            # h2 = attention(h2, name=prefix+'attention_dis_2')
            
            # h2 is (32x 32 x self.df_dim*4)
            h3 = conv2d(h2, self.df_dim*8, d_h=1, d_w=1, name=prefix+'h3_conv')
            h3 = batch_norm(h3, name = prefix+ 'bn3')
            h3 = lrelu(h3)
            # h3 = attention(h3, name=prefix+'attention_dis_3')
            
            # h3 is (32 x 32 x self.df_dim*8)
            h4 = conv2d(h3, 1, d_h=1, d_w=1, name =prefix+'h4')
            # h4 = attention(h4, name=prefix+'attention_dis_4')
            return h4
    ...
    def fcn(self, imgs, prefix=None, reuse = False):
        with tf.variable_scope(tf.get_variable_scope()) as scope:
            ...
            e1 = lrelu(e1)
            e1 = attention(e1, name=prefix+'attention_gen_1')

            # e1 is (128 x 128 x self.fcn_filter_dim)
            e2 = conv2d(e1, self.fcn_filter_dim*2, name=prefix+'e2_conv')
            e2 = batch_norm(e2, name = prefix+'bn_e2')
            e2 = lrelu(e2)
            e2 = attention(e2, name=prefix+'attention_gen_2')

            # e2 is (64 x 64 x self.fcn_filter_dim*2)
            e3 = conv2d(e2, self.fcn_filter_dim*4, name=prefix+'e3_conv')
            e3 = batch_norm(e3, name = prefix+'bn_e3')
            e3 = lrelu(e3)
            e3 = attention(e3, name=prefix+'attention_gen_3')
            
            # e3 is (32 x 32 x self.fcn_filter_dim*4)
            e4 = conv2d(e3, self.fcn_filter_dim*8, name=prefix+'e4_conv')
            e4 = batch_norm(e4, name = prefix+'bn_e4')
            e4 = lrelu(e4)
            e4 = attention(e4, name=prefix+'attention_gen_4')
            
            # e4 is (16 x 16 x self.fcn_filter_dim*8)
            e5 = conv2d(e4, self.fcn_filter_dim*8, name=prefix+'e5_conv')
            e5 = batch_norm(e5, name = prefix+'bn_e5')
            e5 = lrelu(e5)
            
            # e5 = attention(e5, name=prefix+'attention_gen_5')
            e6 = conv2d(e5, self.fcn_filter_dim*8, name=prefix+'e6_conv')
            e6 = batch_norm(e6, name = prefix+'bn_e6')
            e6 = lrelu(e6)
            
            # e6 is (4 x 4 x self.fcn_filter_dim*8)
            e7 = conv2d(e6, self.fcn_filter_dim*8, name=prefix+'e7_conv')
            e7 = batch_norm(e7, name = prefix+'bn_e7')
            e7 = lrelu(e7)
            
            # e7 is (2 x 2 x self.fcn_filter_dim*8)
            e8 = conv2d(e7, self.fcn_filter_dim*8, name=prefix+'e8_conv')
            e8 = batch_norm(e8, name = prefix+'bn_e8')
    ...
    def test_domain(self, args, test_log, type = 'A'):
        ...
        if type == 'A':
            for i in xrange(0, len(test_files)//self.batch_size):
                ...
                # e2, e4=self.sess.run([self.e2, self.e4],feed_dict={self.real_A: A_imgs, self.real_B:A_imgs})
                # scipy.misc.imsave(filename_o+'_e2.jpg', e2)
                # save_images(e2, [self.batch_size, 1], filename_o+'_e2.jpg')
                # save_images(e4, [self.batch_size, 1], filename_o+'_e4.jpg')
