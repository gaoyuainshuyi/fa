...
class ContentEncoder(nn.Module):
    def __init__(self, n_downsample, n_res, input_dim, dim, norm, activ, pad_type):
        ...
        for i in range(n_downsample):
            ...
            dim *= 2
        # residual blocks
        self.model += [ResBlocks_Encoder(n_res, dim, norm=norm, activation=activ, pad_type=pad_type)]
        self.model += [attention(dim)]
        self.model = nn.Sequential(*self.model)
        self.output_dim = dim
...
class Decoder(nn.Module):
    def __init__(self, n_upsample, n_res, dim, output_dim, res_norm='adain', activ='relu', pad_type='zero'):
        ...
        self.model += [ResBlocks_Decoder(n_res, dim, res_norm, activ, pad_type=pad_type)]
        self.model += [attention(dim)]
        # upsampling blocks
...
# Sequential Models
##################################################################################
class ResBlocks_Encoder(nn.Module):
    def __init__(self, num_blocks, dim, norm='in', activation='relu', pad_type='zero'):
        super(ResBlocks_Encoder, self).__init__()
        self.model = []
...
class ResBlocks_Decoder(nn.Module):
    def __init__(self, num_blocks, dim, norm='in', activation='relu', pad_type='zero'):
        super(ResBlocks_Decoder, self).__init__()
        self.model = []
        for i in range(num_blocks):
            self.model += [ResBlock(dim, norm=norm, activation=activation, pad_type=pad_type)]
        self.model = nn.Sequential(*self.model)
